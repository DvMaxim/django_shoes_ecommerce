...
class ItemDetailView(DetailView):
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if count <= 0:
                messages.info(self.request, "Кажется вы ошиблись с количеством. Пожалуйста, проверьте еще раз и повторите ввод.")

                return reverse("core:product", kwargs={
                    'slug': slug
                })
            else:
                if 'add_cart' in self.request.POST:
                    return reverse("core:add-to-cart",
                                   kwargs={'slug': slug, 'count': count})

                elif 'delete_cart' in self.request.POST:
...
class UserProfileView(DetailView):
    model = UserProfile
    template_name = 'home.html'     # change in future
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            if order_item.quantity > count:
                ...
                order_item.delete()
                messages.info(request, "Товар был полностью удален из корзины.")
            return redirect("core:product", slug=slug)
        else:
